/**
 * PHIÊN BẢN 4 - SỬA LỖI HOÀN TOÀN
 * - Bắt đầu từ hàng 3 (bỏ qua header hàng 1 và 2)
 * - Dùng dấu ; thay vì , cho Google Sheets tiếng Việt
 */
...
const CONFIG = {
  ...
  startRow: 3,  // Dữ liệu bắt đầu từ hàng 3
  lastRow: 500
};
...
function createOverviewSheet() {
  ...
  // KPI formulas - QUAN TRỌNG: dùng dấu ; và bắt đầu từ hàng 3
  sheet.getRange("A4").setFormula(`=COUNTA('${tl}'!A${sr}:A${lr})-COUNTIF('${tl}'!A${sr}:A${lr};"*Support*")-COUNTIF('${tl}'!A${sr}:A${lr};"*BackEnd*")-COUNTIF('${tl}'!A${sr}:A${lr};"*Frontend*")`);
  sheet.getRange("B4").setFormula(`=COUNTIF('${tl}'!G${sr}:G${lr};"Finished")+COUNTIF('${tl}'!G${sr}:G${lr};"Closed")`);
  sheet.getRange("C4").setFormula(`=COUNTIF('${tl}'!G${sr}:G${lr};"In Progress")`);
  sheet.getRange("D4").setFormula(`=COUNTIF('${tl}'!G${sr}:G${lr};"Testing")`);
  sheet.getRange("E4").setFormula(`=COUNTIF('${tl}'!G${sr}:G${lr};"Open")+COUNTIF('${tl}'!G${sr}:G${lr};"Pending")`);
  sheet.getRange("F4").setFormula(`=COUNTIFS('${tl}'!F${sr}:F${lr};"Urgent";'${tl}'!G${sr}:G${lr};"<>Finished";'${tl}'!G${sr}:G${lr};"<>Closed")`);
  ...
  statuses.forEach((s, i) => {
    ...
    sheet.getRange(row, 2).setFormula(`=COUNTIF('${tl}'!G${sr}:G${lr};"${s[1]}")`);
  ...
  priorities.forEach((p, i) => {
    ...
    sheet.getRange(row, 7).setFormula(`=COUNTIF('${tl}'!F${sr}:F${lr};"${p[1]}")`);
    sheet.getRange(row, 8).setFormula(`=COUNTIFS('${tl}'!F${sr}:F${lr};"${p[1]}";'${tl}'!G${sr}:G${lr};"<>Finished";'${tl}'!G${sr}:G${lr};"<>Closed")`);
  ...
  CONFIG.assignees.forEach((name, i) => {
    ...
    sheet.getRange(row, 2).setFormula(`=COUNTIF('${tl}'!H${sr}:H${lr};"*${name}*")`);
  ...
  sheet.getRange("E18").setFormula(
    `=IFERROR(FILTER({'${tl}'!A${sr}:A${lr};'${tl}'!B${sr}:B${lr};'${tl}'!H${sr}:H${lr};'${tl}'!F${sr}:F${lr};'${tl}'!D${sr}:D${lr};'${tl}'!E${sr}:E${lr};'${tl}'!G${sr}:G${lr}};` +
    `('${tl}'!D${sr}:D${lr}<>"")*(('${tl}'!D${sr}:D${lr}-TODAY())<=3)*(('${tl}'!D${sr}:D${lr}-TODAY())>=-7)*` +
    `('${tl}'!G${sr}:G${lr}<>"Finished")*('${tl}'!G${sr}:G${lr}<>"Closed")*('${tl}'!A${sr}:A${lr}<>""));` +
  ...
  CONFIG.assignees.forEach((name, i) => {
    ...
    // Done
    sheet.getRange(row, 3).setFormula(`=COUNTIFS('${tl}'!H${sr}:H${lr};"*${name}*";'${tl}'!G${sr}:G${lr};"Finished")+COUNTIFS('${tl}'!H${sr}:H${lr};"*${name}*";'${tl}'!G${sr}:G${lr};"Closed")`);
    
    // In Progress
    sheet.getRange(row, 4).setFormula(`=COUNTIFS('${tl}'!H${sr}:H${lr};"*${name}*";'${tl}'!G${sr}:G${lr};"In Progress")`);
    
    // Testing
    sheet.getRange(row, 5).setFormula(`=COUNTIFS('${tl}'!H${sr}:H${lr};"*${name}*";'${tl}'!G${sr}:G${lr};"Testing")`);
    
    // Pending
    sheet.getRange(row, 6).setFormula(`=COUNTIFS('${tl}'!H${sr}:H${lr};"*${name}*";'${tl}'!G${sr}:G${lr};"Open")+COUNTIFS('${tl}'!H${sr}:H${lr};"*${name}*";'${tl}'!G${sr}:G${lr};"Pending")`);
    
    // Urgent (chưa xong)
    sheet.getRange(row, 7).setFormula(`=COUNTIFS('${tl}'!H${sr}:H${lr};"*${name}*";'${tl}'!F${sr}:F${lr};"Urgent";'${tl}'!G${sr}:G${lr};"<>Finished";'${tl}'!G${sr}:G${lr};"<>Closed")`);
    
    // High
    sheet.getRange(row, 8).setFormula(`=COUNTIFS('${tl}'!H${sr}:H${lr};"*${name}*";'${tl}'!F${sr}:F${lr};"High";'${tl}'!G${sr}:G${lr};"<>Finished";'${tl}'!G${sr}:G${lr};"<>Closed")`);
    
    // Normal
    sheet.getRange(row, 9).setFormula(`=COUNTIFS('${tl}'!H${sr}:H${lr};"*${name}*";'${tl}'!F${sr}:F${lr};"Normal";'${tl}'!G${sr}:G${lr};"<>Finished";'${tl}'!G${sr}:G${lr};"<>Closed")`);
    
    // Low
    sheet.getRange(row, 10).setFormula(`=COUNTIFS('${tl}'!H${sr}:H${lr};"*${name}*";'${tl}'!F${sr}:F${lr};"Low";'${tl}'!G${sr}:G${lr};"<>Finished";'${tl}'!G${sr}:G${lr};"<>Closed")`);
    
    // Task đang làm
    sheet.getRange(row, 11).setFormula(
      `=IFERROR(TEXTJOIN("; ";TRUE;FILTER('${tl}'!A${sr}:A${lr}&"-"&'${tl}'!B${sr}:B${lr};` +
      `(ISNUMBER(SEARCH("${name}";'${tl}'!H${sr}:H${lr})))*('${tl}'!G${sr}:G${lr}="In Progress")));"Không có")`
    );
  });
